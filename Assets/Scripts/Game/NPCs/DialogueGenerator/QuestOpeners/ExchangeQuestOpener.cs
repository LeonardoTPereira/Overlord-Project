        ...
        protected override string[] lowSocialDialogues
        {
            get {
                if (_language == Util.Enums.Language.Portuguese)
                    ...
                    "Por que você não vai incomodar outra pessoa, hein? Leve um {questSo.GetItemString()} até {questSo.GetTargetNpc()}. Ele deve ter algo esperando para quem entregar—não me pergunte o quê, eu já esqueci.",
                    ...
                    "Se você está tão ansioso por recompensas, leve um {questSo.GetItemString()} até {questSo.GetTargetNpc()}. Ele vãi te dar algo, tenho certeza.",
                    "Eu estou muito ocupado para pedidos de troca. Leve um {questSo.GetItemString()} até {questSo.GetTargetNpc()}, você será recompensado, e eu não terei que levantar um dedo."
                    };
                return new string[] {
                ...
                "Why don't you go bother someone else, will ya? Take a {questSo.GetItemString()} over to {questSo.GetTargetNpc()}. They’ve got something waiting for whoever delivers it—don’t ask me what.",
        ...
        protected override string[] averageSocialDialogues
        {
            get {
                if (_language == Util.Enums.Language.Portuguese)
                    ...
                    "Eu deveria ter dado {questSo.GetTargetNpc()} um {questSo.GetItemString()} antes do amanhecer para o novo feitiço dele, mas acho que não vou conseguir. Se você puder fazer isso por mim, pode pegar a recompensa deles.",
                    "Você poderia levar um {questSo.GetItemString()} até {questSo.GetTargetNpc()}? Ele me disse que esta procurando isso e tem uma recompensa para quem entregar."
                    };
                return new string[] {
        ...
        protected override string[] highSocialDialogues
        {
            get {
                if (_language == Util.Enums.Language.Portuguese)
                    ...
                    "Eu estava querendo trocar um {questSo.GetItemString()} com {questSo.GetTargetNpc()}, mas estou tão ocupado! Se você fizer isso por mim, tenho certeza de que ele vai te dar algo incrível. Ele sempre tem os melhores tesouros!",
                    "Você é a pessoa certa para isso! Leve um {questSo.GetItemString()} até {questSo.GetTargetNpc()}, e provavelmente ele vai te dar algo melhor do que você imaginava. {questSo.GetTargetNpc()} sempre me surpreendem com o que tem!",
                    "Ah, eu sei que você está ocupado, mas {questSo.GetTargetNpc()} vai realmente apreciar uma troca! {questSo.GetTargetNpc()} sempre recompensam a gentileza—acredite, vai valer a pena o seu tempo! É só levar um {questSo.GetItemString()} até ele e você vai ver o que eu quero dizer!",
                    "Ei, você poderia trocar um {questSo.GetItemString()} com {questSo.GetTargetNpc()}? Não vejo a hora de ver a expressão no rosto do {questSo.GetTargetNpc()} quando você entregar isso! Ele provavelmente vai te recompensar com algo que vai te deixar de queixo caído. {questSo.GetTargetNpc()} sempre têm as melhores coisas!"
                    };
                return new string[] {
